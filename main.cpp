...
double Calheuristic(const vector<int>& state){
...
// Heuristic search use misplaced tiles cost
// x = 3 to use euclidean distance cost
void misplaced_heuristic_search(Problem& problem){
    // priority_queue<Node> frontier;
    // set<vector<int> > explored;

    // double initial_heuristic = Calheuristic(problem.initial_state, problem.goal_state, 2);
    ...
    //     if (nodes_expanded > 0)
    //     {
    //         cout << "The best state to expand with g(n) = " << node.path_cost << " and h(n) = " << node.heuristic_cost << " is...";
    //     }
    //     print_state(node.state);
    ...
    //     vector<string> possible_moves = problem.get_possible_moves(node.state);
    //     // cout << possible_moves[1];
    //     for (size_t i = 0; i < possible_moves.size(); i++) {
    //         const string& move = possible_moves[i];
            
    //         vector<int> new_state = problem.apply_operator(node.state, move);

    //         if (explored.find(new_state) == explored.end()) {
    //             double new_heuristic = Calheuristic(new_state, problem.goal_state, 2);
    //             Node child(new_state, &node, move, node.path_cost + 1, new_heuristic);
    //             frontier.push(child);
    //             // max_queue_size++;
    //         }
    //     }
...
void euclidean_heuristic_search(Problem& problem){
    // priority_queue<Node> frontier;
    // set<vector<int> > explored;

    // double initial_heuristic = Calheuristic(problem.initial_state, problem.goal_state, 3);
    // frontier.push(Node(problem.initial_state, nullptr, "", 0, initial_heuristic));
    // int nodes_expanded = 0;
    // int max_queue_size = 1;
    ...
    //         if (explored.find(new_state) == explored.end()) {
    //             double new_heuristic = Calheuristic(new_state, problem.goal_state, 3);
    //             Node child(new_state, &node, move, node.path_cost + 1, new_heuristic);
    //             frontier.push(child);
    //             // max_queue_size++;
    //         }
...
int main() {
    ...
    {
        misplaced_heuristic_search(puzzle_problem);

    }else if (methodChoice == 3)
    {
        euclidean_heuristic_search(puzzle_problem);
    }
